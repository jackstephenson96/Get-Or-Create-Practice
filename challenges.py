...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/discussion_songs_db"
...
def get_or_create_artist(artist_name):
	a = db.session.query(Artist).filter_by(name=artist_name).first()
	if a:
		return a
	else:
...
def get_or_create_song(song_title, song_artist, song_genre):
	s = db.session.query(Song).filter_by(title=song_title).first()
	if s:
		return s
	else:
		artist = get_or_create_artist(song_artist)
...
@app.route('/all_songs')
def see_all():
	return render_template('all_songs.html', all_songs={(song.title, str(Artist.query.filter_by(id=song.artist_id).first()).split("(")[0], song.genre) for song in Song.query.all()})

@app.route('/all_artists')
def see_all_artists():
	artists = Artist.query.all()
	names = [a.name, len(Song.query.filter_by(artist_id=a.id).all() for a in artists)]
	return render_template('all_artists', artist_names=names)
